...
export function InventoryWindow({ onClose }: { onClose: () => void }) {
  ...
  return (
    <WindowFrame title="Inventario" onClose={onClose} style={{ right: '10px', top: '10px', width: '280px', height: '360px' }}>
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-2 font-sans">
        {player.inventory.length === 0 ? (
          ...
          player.inventory.map((item) => (
             <div key={item.id} className="p-2 border border-slate-300 bg-white rounded-md relative flex justify-between items-start cursor-pointer hover:bg-slate-50 transition-colors">
               ...
               {item.type === 'usable' && (
                  <button onClick={() => consumeItem(item.id)} className="bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm hover:bg-blue-500 active:translate-y-px">Usar</button>
               )}
             </div>
          ))
        )}
